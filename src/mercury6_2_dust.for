...
c Author: M. Fenucci 
c 
c Reads the content of the file dust.nml for parameters of radiation
c forces
c
c------------------------------------------------------------------------------
      ...
      subroutine dust_in
         ...
         ! Echo what was read
         write(*,*) 'rp_rho [kg/m^3]  = ', rp_rho
         write(*,*) 'rp_s   [m]       = ', rp_s
         write(*,*) 'rp_QPR           = ', rp_QPR
         write(*,*) 'rp_csi           = ', rp_csi
         write(*,*) 'rp_beta          = ', rp_beta
...
c
      ...
      do j = 2, nbod
        ...
        r2 = dx**2 + dy**2 + dz**2 
        r  = sqrt(r2)
        ! Compute cross(x, v) and the norm
        rxv(1) = dy*v3 - dz*v2
        rxv(2) = dz*v1 - dx*v3
        rxv(3) = dx*v2 - dy*v1
        norm_rxv = sqrt(rxv(1)**2 + rxv(2)**2 + rxv(3)**2)
...
c
      if (opt(3).eq.0.or.opt(3).eq.2) then
        ...
        flog = '(1x,a,i15,1x,i2,1x,f4.1,2(a,1p1e12.5))'
      else
        tstring = mem(2)
