...
module yorp_module
   ...
   ! INPUT
   ! astname : name of the asteroid
   !      t2 : second integration time
   !    step : stepsize of integration (NOTE: t1 = t2 - h)
   !      t0 : initial integration time
   !   t_out : time in output
   !      y1 : spin solution at t1
   !      y2 : spin solution at t2
   !    dadt : semimajor axis drift
   subroutine yorp_output(astname, t2, step, t0, t_out, y1, y2, dadt)
      character*15, intent(in)     :: astname
      real(kind=dkind), intent(in) :: t2, step, t0
      real(kind=dkind), intent(in) :: t_out
      real(kind=dkind), intent(in) :: y1(2), y2(2)
      real(kind=dkind), intent(in) :: dadt
      ! end interface
      real(kind=dkind) :: t1, t_aux
      real(kind=dkind) :: y_out(2)
      ! Compute t1
      t1 = t2 - step - t0
